...
| **SAF-02** | Cắt quá áp bằng Firmware | Ghi đè code bơm liên tục. Chặn không cho van xả. | Khi áp suất ADC đọc được vượt quá **trần SAF hiệu lực** (mặc định 175 mmHg, có thể đổi UART `SAF`/`SAFH`), Firmware phải tự động ngắt Bơm và mở Van 100% (Safety Limit). |
